...
/**
 * Implementação do DAO do objeto TerminalPOS para o SGBD Hsqldb
 * @author Edison Klafke Fillus
 * @author Jefferson de Souza Campos
 */
public class HsqldbTerminalPOSDAO extends HsqldbDAOFactory implements TerminalPOSDAO {
    ...
    public TerminalPOS findTerminalPOS(int id) {
        ...
        try {
            ...
            EntityManager em = super.getJPAEntityManager(); //Recupera o EntityManager
            terminalPOS = em.find(TerminalPOS.class, id); //Localiza terminalPOS
            em.refresh(terminalPOS);
            logger.info("Objeto " + CLASS_NAME + " consultado: " + terminalPOS.toString());
        } catch (Exception ex) {
